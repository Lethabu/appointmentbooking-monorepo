#!/bin/bash
# =============================================================================
# Final Working WSL Deployment Script
...
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  AppointmentBooking.co.za - Final WSL Deployment                ║"
echo "║  Simplified and Working                                          ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo ""

BOOKING_DIR="/mnt/c/Users/Adrin/Documents/MyProjects/appointmentbooking-monorepo/apps/booking"
...
# Verify .next directory exists from previous build
if [ ! -d ".next" ]; then
    echo "❌ ERROR: .next directory not found. Next.js build required first."
    echo "   Run: pnpm run build"
    exit 1
fi
echo "✅ .next directory exists from previous build"
echo ""

# Run Cloudflare Pages adapter with correct pnpm syntax
echo "⚡ Building Cloudflare Pages adapter..."
echo "   Converting Next.js build to Cloudflare Workers format..."
rm -rf .vercel

# Force environment variables and use npx to ensure latest version
WRANGLER_COMPATIBILITY_DATE=2024-09-23 WRANGLER_COMPATIBILITY_FLAGS=nodejs_compat npx -y @cloudflare/next-on-pages@latest
