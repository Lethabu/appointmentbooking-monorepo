...
import { requireCustomerOrAdmin } from '../../../lib/auth'; // Adjust path as needed
...
export const POST = requireCustomerOrAdmin(async (req: NextRequest) => {
  try {
    const { salonId, clientDetails, cart, total }: OrderRequestBody = await req.json();
    ...
    // Return a generic error message to prevent leakage of sensitive internal details
    return new NextResponse(
